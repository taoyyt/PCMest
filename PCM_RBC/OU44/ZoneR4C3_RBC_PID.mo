within OU44;
model ZoneR4C3_RBC_PID "single zone model"
  ...
  Modelica.Blocks.Interfaces.RealInput Tvestp
    "indoor temperature setpoint [deg C]"
    annotation (Placement(transformation(extent={{-158,-96},{-140,-78}})));
  ...
  Modelica.Blocks.Continuous.LimPID PID_temp(
    ...
    Ti=600,
    yMin=-1)
    annotation (Placement(transformation(extent={{-110,-94},{-96,-80}})));
  ...
  Modelica.Thermal.HeatTransfer.Components.HeatCapacitor HC_air(C=Cair, T(
    ...
    annotation (Placement(transformation(extent={{4,88},{22,106}})));
  ...
  Modelica.Blocks.Math.Gain heating(k=maxHeat)
    annotation (Placement(transformation(extent={{-68,-94},{-50,-76}})));
...
equation
  ...
  connect(prescribedTemperature.port, Re.port_a)
    annotation (Line(points={{-86,81},{-82,81},{-82,79},{-78,79}},
                                                 color={191,0,0}));
  connect(HC_air.port, Rinternal.port_a) annotation (Line(points={{13,88},{26,
          88},{26,108},{40,108}},
                              color={191,0,0}));
  ...
  connect(Rinfiltration.port_b,HC_air. port) annotation (Line(points={{-18,97},
          {0,97},{0,88},{13,88}},color={191,0,0}));
  connect(HC_air.port, temperatureSensor.port)
    annotation (Line(points={{13,88},{34,88}}, color={191,0,0}));
  ...
  connect(occHeatGain.port,HC_air. port)
    annotation (Line(points={{2,26},{13,26},{13,88}}, color={191,0,0}));
  ...
  connect(Tvestp, PID_temp.u_s)
    annotation (Line(points={{-149,-87},{-111.4,-87}}, color={0,0,127}));
  connect(PID_temp.y, heating.u) annotation (Line(points={{-95.3,-87},{-84,-87},
          {-84,-85},{-69.8,-85}}, color={0,0,127}));
  connect(heating.y, prescribedHeatFlow1.Q_flow) annotation (Line(points={{-49.1,
          -85},{-40,-85}},                           color={0,0,127}));
  connect(temperatureSensor.T, fromKelvin1.Kelvin) annotation (Line(points={{46,
          88},{54,88},{54,-102},{-22.8,-102}}, color={0,0,127}));
  ...
  connect(heatFlowSensor.port_b,HC_air. port) annotation (Line(points={{-4,-85},
          {20,-85},{20,88},{13,88}}, color={191,0,0}));
  ...
  connect(PID_temp.y, scale1.u) annotation (Line(points={{-95.3,-87},{-83.65,
          -87},{-83.65,-114},{40.8,-114}}, color={0,0,127}));
  connect(scale1.y, vpos)
    annotation (Line(points={{54.6,-114},{130,-114}}, color={0,0,127}));
  connect(vpos, vpos)
    annotation (Line(points={{130,-114},{130,-114}}, color={0,0,127}));
  ...
  connect(HC_air.port, airMix.port_b) annotation (Line(points={{13,88},{20,88},
  ...
  connect(Re.port_b, Rindoor.port_a)
    annotation (Line(points={{-60,79},{-30,79}}, color={191,0,0}));
  connect(Rindoor.port_b,HC_air. port) annotation (Line(points={{-16,79},{0,79},
          {0,88},{13,88}}, color={191,0,0}));
  connect(Re.port_b,HC_wall. port)
    annotation (Line(points={{-60,79},{-50,79},{-50,69}}, color={191,0,0}));
  ...
  connect(fromKelvin1.Celsius, PID_temp.u_m) annotation (Line(points={{-36.6,
          -102},{-103,-102},{-103,-95.4}}, color={0,0,127}));
