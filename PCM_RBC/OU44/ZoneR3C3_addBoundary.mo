within OU44;
model ZoneR3C3_addBoundary "single zone model"
  ...
  Modelica.Blocks.Continuous.Integrator integrator1(k=1)
    annotation (Placement(transformation(extent={{84,-112},{98,-98}})));
  ...
  Modelica.Blocks.Math.Gain scale2(k=0.01)
        ...
        rotation=180,
        origin={-103,-85})));
  Modelica.Blocks.Interfaces.RealInput Tbound
    "indoor boundary t temperature (deg C)" annotation (Placement(
        transformation(
        extent={{-7,-7},{7,7}},
        rotation=0,
        origin={-149,95})));
  Modelica.Thermal.HeatTransfer.Celsius.ToKelvin toKelvin2
    annotation (Placement(transformation(extent={{-134,90},{-124,100}})));
  Modelica.Thermal.HeatTransfer.Sources.PrescribedTemperature
    prescribedTemperature1
    annotation (Placement(transformation(extent={{-116,92},{-110,98}})));
equation
  ...
  connect(integrator1.y, Qrad) annotation (Line(points={{98.7,-105},{110.35,
          -105},{110.35,-101},{137,-101}},
                                   color={0,0,127}));
...
connect(heatFlowSensor.Q_flow, integrator1.u) annotation (Line(points={{-11,-92},
          {34,-92},{34,-105},{82.6,-105}},    color={0,0,127}));
connect(airMix.port_b, HC_air.port) annotation (Line(points={{8,-21},{14,-21},
        {14,-18},{20,-18},{20,88},{15,88}}, color={191,0,0}));
  connect(Tbound, toKelvin2.Celsius)
    annotation (Line(points={{-149,95},{-135,95}}, color={0,0,127}));
  connect(toKelvin2.Kelvin, prescribedTemperature1.T)
    annotation (Line(points={{-123.5,95},{-116.6,95}}, color={0,0,127}));
  connect(prescribedTemperature1.port, Re.port_a)
    annotation (Line(points={{-110,95},{-78,95},{-78,81}}, color={191,0,0}));
  annotation (
